...
public class RabbitMQConfig {
    ...
    public Queue notificacaoQueue() {
        return QueueBuilder
                .durable(notificacaoQueue)
                .withArgument("x-dead-letter-exchange", "") // DLQ para mensagens com falha
                .withArgument("x-dead-letter-routing-key", notificacaoQueue + ".dlq")
                .build();
    }
    ...
    public Queue notificacaoDeadLetterQueue() {
        return QueueBuilder
                .durable(notificacaoQueue + ".dlq")
                .build();
    }

    /**
     * Binding que conecta a fila do histórico ao exchange
     * Todas as mensagens com routing key 'consulta.notificacao' serão direcionadas para esta fila
     */
    @Bean
     ...
     */
    @Bean
    public Binding notificacaoAllEventsBinding() {
        return BindingBuilder
                .bind(notificacaoQueue())
                .to(consultasExchange())
